...
impl BlobBuilder {
    ...
    /// Consumes the builder and returns a finalized [`Blob`](libvctrl_handler::Blob).
    ///
    /// # Design rationale
    /// This method consumes `self` to enforce a linear flow—the builder cannot
    /// be reused after the data has been extracted. The internal `Vec<u8>` is
    /// moved directly into the `Blob` without cloning, ensuring zero-cost
    /// finalization.
    ///
    /// # Examples
    ///
